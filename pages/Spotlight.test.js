...
test("renders an image on page load", () => {
  render(<HomePage art={dummyData} favoriteArts={dummyDataFavorites} />);
  const image = screen.getByAltText("sptlightArt");
  expect(image).toBeInTheDocument();
});

test("renders the artist's name", () => {
  jest.spyOn(Math, "random").mockReturnValue(0);
  render(<HomePage art={dummyData} favoriteArts={dummyDataFavorites} />);
  const artistName = screen.getByText("Vincent van Gogh");
...
test("renders the favorite button", () => {
  jest.spyOn(Math, "random").mockReturnValue(0);
  render(<HomePage art={dummyData} favoriteArts={dummyDataFavorites} />);
  const favoriteButton = document.querySelector("svg");
